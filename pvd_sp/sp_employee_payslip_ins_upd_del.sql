...
/****** Object:  StoredProcedure [dbo].[sp_employee_payslip_ins_upd_del]    Script Date: 16-12-2025 16:20:24 ******/
SET ANSI_NULLS ON
GO
...
@action varchar(max) = '',
@emp_payslip_id bigint=0,
@fin_year_id bigint=0,
@month_id bigint=0,
@company_id bigint=0,
@employee_id bigint=0,
@emp_calender_id bigint=0,
@employee_working_days decimal(4,2)=0,
@actual_working_days decimal(4,2)=0,
@actual_working_hours decimal(5,2)=0,
@monthly_salary decimal(12,2)=0,
@hourly_salary decimal(12,2)=0,
@over_time_hours decimal(5,2)=0,
@over_time_amount decimal(12,2)=0,
@gross_amount decimal(12,2)=0,
@pf_amount decimal(12,2)=0,
@tds_amount decimal(12,2)=0,
@advance_amount decimal(12,2)=0,
@netamount decimal(12,2)=0,
@remarks varchar(100)='',
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			set @actual_working_days = isnull((@employee_working_days - (Select count(1)
																	...
																	comp_id = @company_id and 
																	month_id = @month_id
																	)),0);
			set @actual_working_hours = isnull((@actual_working_days * 8),0);
			set @monthly_salary = isnull(((Select annual_salary from employee_mast where employee_id = @employee_id and fin_year_id=@fin_year_id and comp_id=@company_id)/12),0);
			...
			@pf_amount,@tds_amount,@advance_amount,@netamount,@remarks,@created_date,@updated_date,@created_by,@modified_by);
			commit transaction;
		end try
		begin catch
			IF XACT_STATE() <> 0
	...
	begin
		begin try
			...
			left join user_mast um on ep.created_by = um.user_id
	...
	begin
		begin try
			...
			set @payment_id = isnull((select payment_id from payment_mast where emp_payslip_id = @emp_payslip_id and fin_year_id=@fin_year_id and comp_id=@company_id),0)
