...
/****** Object:  StoredProcedure [dbo].[sp_state_mast_ins_upd_del]    Script Date: 04-11-2025 10:58:01 ******/
SET ANSI_NULLS ON
GO
...
if @action = 'insert'
	...
	begin
		begin try
			...
			from state_mast sm
			inner join country_mast cm on sm.country_id = cm.country_id
			inner join user_mast um on sm.user_id=um.user_id;	
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
