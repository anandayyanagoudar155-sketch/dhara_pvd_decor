...
/****** Object:  StoredProcedure [dbo].[sp_vendor_mast_ins_upd_del]    Script Date: 04-11-2025 17:17:27 ******/
SET ANSI_NULLS ON
GO
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			from vendor_mast vm
			inner join user_mast um on vm.user_id = um.user_id
			inner join city_mast cm on vm.city_id = cm.city_id;
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
