...
/****** Object:  StoredProcedure [dbo].[sp_purchaseinvoice_mast_ins_upd_del]    Script Date: 04-11-2025 17:18:44 ******/
SET ANSI_NULLS ON
GO
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			sgst_total,cgst_total,igst_total,discount_total,roundoff_total,net_total,paymentstatus,
			...
			@cgst_total,@igst_total,@discount_total,@roundoff_total,@net_total,@paymentstatus,
			@is_active,@fin_year_id,@comp_id,@created_date,@updated_date,@user_id);
			commit transaction;
		end try
		begin catch
	...
	begin
		begin try
			...
			pim.net_total,pim.paymentstatus,pim.is_active,fym.fin_name,cm.comp_name,pim.created_date,
			pim.updated_date,um.user_name
			from purchaseinvoice_mast pim
			inner join user_mast um on pim.user_id = um.user_id
			inner join company_mast cm on pim.comp_id = cm.comp_id
			inner join fin_year_mast fym on pim.fin_year_id = fym.fin_year_id
			inner join vendor_mast vm on pim.vendor_id = vm.vendor_id;
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
