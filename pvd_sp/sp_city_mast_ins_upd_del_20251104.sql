...
/****** Object:  StoredProcedure [dbo].[sp_city_mast_ins_upd_del]    Script Date: 04-11-2025 10:58:30 ******/
SET ANSI_NULLS ON
GO
...
if @action = 'insert'
	...
	begin
		begin try
			...
			from city_mast cm
			inner join state_mast sm on cm.state_id = sm.state_id
			inner join user_mast um on cm.user_id = um.user_id;	
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
