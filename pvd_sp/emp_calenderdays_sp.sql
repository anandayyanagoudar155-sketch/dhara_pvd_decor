...
GO
/****** Object:  StoredProcedure [dbo].[sp_emp_calenderdays_ins_upd_del]    Script Date: 11/8/2025 5:16:53 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER procedure [dbo].[sp_emp_calenderdays_ins_upd_del]
(
@action varchar(max) = '',
@emp_calender_id bigint = 0,
@emp_calender_code varchar(15)='',
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			begin transaction
			insert into emp_calenderdays(emp_calender_code,fin_year_id,month_id,month_days,emp_holidays,emp_weekend,created_date,updated_date,user_id)
			values(@emp_calender_code,@fin_year_id,@month_id,@month_days,@emp_holidays,@emp_weekend,@created_date,@updated_date,@user_id);
			commit transaction;
		end try
		begin catch
			if XACT_STATE() <> 0
	...
	begin
		begin try
			Select ecd.emp_calender_id,ecd.emp_calender_code,fym.fin_name,mm.month_name,ecd.month_days,ecd.emp_holidays,
	...
	begin
		begin try
			Select ecd.emp_calender_id,ecd.emp_calender_code,ecd.fin_year_id,ecd.month_id,ecd.month_days,ecd.emp_holidays,
	...
	begin
		begin try
			...
			set emp_calender_code=@emp_calender_code,
		...
		if @action = 'emp_calander_list'
		begin
			begin try
				Select emp_calender_id,emp_calender_code
