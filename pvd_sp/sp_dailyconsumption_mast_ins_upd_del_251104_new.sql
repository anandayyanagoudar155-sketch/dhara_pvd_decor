...
/****** Object:  StoredProcedure [dbo].[sp_dailyconsumption_mast_ins_upd_del]    Script Date: 04-11-2025 17:19:49 ******/
SET ANSI_NULLS ON
GO
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			inner join user_mast um on dcm.user_id = um.user_id
			inner join company_mast cm on dcm.comp_id = cm.comp_id
			inner join fin_year_mast fym on dcm.fin_year_id = fym.fin_year_id
			inner join product_mast pm on dcm.product_id = pm.product_id
			inner join unit_mast utm on dcm.unit_id = utm.unit_id;
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
