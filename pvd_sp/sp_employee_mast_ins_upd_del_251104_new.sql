...
/****** Object:  StoredProcedure [dbo].[sp_employee_mast_ins_upd_del]    Script Date: 04-11-2025 17:16:40 ******/
SET ANSI_NULLS ON
GO
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			inner join user_mast um on em.user_id = um.user_id
			inner join employee_desg_mast edm on em.desg_id = edm.desg_id
			inner join fin_year_mast fym on em.fin_year_id = fym.fin_year_id
			inner join company_mast cpm on em.comp_id = cpm.comp_id
			inner join city_mast cm on em.city_id = cm.city_id;
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
