...
/****** Object:  StoredProcedure [dbo].[sp_user_mast_ins_upd_del]    Script Date: 03-12-2025 18:26:52 ******/
SET ANSI_NULLS ON
GO
...
begin
	begin try
		begin transaction
		if EXISTS (Select 1 from user_mast where user_name=@user_name and user_role=@user_role)
		Begin
				RAISERROR('Cannot insert: Same username and user_role is present', 16, 1);
				return;
		End
...
begin	
	...
	begin try
		...
		set @rec_count = (select sum(cnt)
			...
			union all
			select count(user_id) as cnt from product_details where user_id = @user_id
			union all
			select count(USER_ID) as cnt from vendor_details where user_id = @user_id
			union all
			select count(USER_ID) as cnt from customer_details where user_id = @user_id
		) as rec_count);
...
begin
	begin try
		...
		if EXISTS (Select 1 from user_mast 
				   where user_name=@user_name 
				   and user_role=@user_role
				   and user_id <> @user_id
				   )
		Begin
				RAISERROR('Incorrect username and role', 16, 1);
				return;
		End
...
BEGIN
    ...
    BEGIN
        ...
        RAISERROR('Login successful', 16, 1)
        RETURN 1;
    END
    ELSE
    BEGIN
