	...
	begin try
		begin transaction
			...
			Begin
					...
					return;
			End
			if EXISTS (Select 1 from customer_mast where customer_name = @customer_name)
			Begin
	...
	begin try
		begin transaction
			...
			Begin
					...
					return;
			End
			if EXISTS (Select 1 from customer_mast where customer_name = @customer_name and customer_id <> @customer_id)
			Begin
					RAISERROR('Cannot insert: customer_name is already present', 16, 1);
					return;
			End
