create procedure sp_receipt_mast_ins_upd_del(
@action varchar(max),
@receipt_id bigint=0,
@sales_id bigint=0,
...
begin
	begin try
		select receipt_id,sales_id,recepit_date,net_total,balance_amount,receipt_status,fin_year_id,comp_id,created_date,updated_date,user_id from receipt_mast; 	
	end try
		begin catch
			set @ErrorNumber = ERROR_NUMBER();
			set @ErrorProcedure = ERROR_PROCEDURE();
