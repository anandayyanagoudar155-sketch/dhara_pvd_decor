...
/****** Object:  StoredProcedure [dbo].[sp_state_mast_ins_upd_del]    Script Date: 12/16/2025 1:17:23 AM ******/
SET ANSI_NULLS ON
GO
...
if @action = 'insert'
	...
	begin
		begin try
			...
			left join user_mast uc on sm.created_by=uc.user_id
			left join user_mast um on sm.modified_by=um.user_id;	
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
