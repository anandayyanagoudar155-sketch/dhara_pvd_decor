...
/****** Object:  StoredProcedure [dbo].[sp_emp_calenderdays_ins_upd_del]    Script Date: 04-11-2025 17:18:09 ******/
SET ANSI_NULLS ON
GO
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			inner join month_mast mm on ecd.month_id = mm.month_id
			inner join fin_year_mast fym on ecd.fin_year_id = fym.fin_year_id
			inner join user_mast um on ecd.user_id = um.user_id;
		end try
		begin catch
			set @errornumber = ERROR_NUMBER();
			set @errorprocedure = ERROR_PROCEDURE();
