...
/****** Object:  StoredProcedure [dbo].[sp_emp_calenderdays_ins_upd_del]    Script Date: 24-11-2025 19:58:42 ******/
SET ANSI_NULLS ON
GO
...
declare @errornumber int, @errorprocedure nvarchar(128), @errorline int, @errormessage nvarchar(max);
	...
	begin
		begin try
			...
			Begin
					RAISERROR('Cannot insert: company_id is incorrect', 16, 1);
					return;
			End
			...
			Begin
					...
					return;
			End
			if not EXISTS (Select 1 from month_mast where month_id = @month_id)
			Begin
	...
	begin
		begin try
			...
			if not EXISTS (Select 1 from company_mast where comp_id = @comp_id)
			Begin
					RAISERROR('Cannot update: comp_id is incorrect', 16, 1);
					return;
			End
			if not EXISTS (Select 1 from fin_year_mast where fin_year_id = @fin_year_id)
			Begin
					RAISERROR('Cannot update: fin_year_id is incorrect', 16, 1);
					return;
			End
			if not EXISTS (Select 1 from month_mast where month_id = @month_id)
			Begin
