...
/****** Object:  StoredProcedure [dbo].[sp_month_mast_ins_upd_del]    Script Date: 24-11-2025 20:00:41 ******/
SET ANSI_NULLS ON
GO
	...
	begin
		begin try
			...
			Begin
					...
					return;
			End
			if EXISTS (Select 1 from month_mast where month_name = @month_name)
			Begin
					RAISERROR('Cannot insert: month_name is already present', 16, 1);
					return;
			End
	...
	begin
		begin try
			...
			Begin
					...
					return;
			End
			if EXISTS (Select 1 from month_mast where month_name = @month_name and month_id<>@month_id)
			Begin
					RAISERROR('Cannot update: month_name is already present', 16, 1);
					return;
			End
			update month_mast
